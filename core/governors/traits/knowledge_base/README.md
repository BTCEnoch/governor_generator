# 🏛️ **ENOCHIAN GOVERNOR KNOWLEDGE BASE & PERSONALITY REFINEMENT SYSTEM**
...
## 🎭 **PERSONALITY REFINEMENT FEATURES**
...
### **For Claude API Batch Processing**
...
## Overview
The knowledge base contains mystical tradition data used by the Governor system. Data is organized in two main ways:

1. Consolidated Format (Preferred)
- Location: `consolidated/`
- Contains complete tradition data in single JSON files
- Example: `enochian_magic.json`
- Includes all aspects: concepts, traits, relationships, etc.
- Higher quality and more complete data

2. Legacy Format (Being Migrated)
- Location: `universal/`, `western/`, `eastern/`, etc.
- Split across multiple files per tradition
- Being gradually migrated to consolidated format

## Data Loading
The system uses knowledge database modules (like `enochian_knowledge_database.py`) to load and process tradition data. These modules:
- Load data from consolidated JSON files
- Convert to proper schema types (KnowledgeEntry, ProcessedTradition)
- Maintain compatibility with existing systems
- Provide search and retrieval functions

## Migration Status
- ✅ Enochian Magic: Fully migrated to consolidated format
- ⏳ Other traditions: To be migrated
- See implementation_dashboard.md for migration schedule

## Adding New Traditions
1. Create consolidated JSON file in `consolidated/`
2. Create knowledge database module
3. Update unified_knowledge_retriever.py
4. Add to implementation_dashboard.md

## Schema Structure
See `core/lighthouse/schemas/knowledge_schemas.py` for full definitions:
- KnowledgeEntry: Individual knowledge items
- ProcessedTradition: Complete tradition data
- ContentQuality: Quality rating enum
- KnowledgeType: Content type classification

## Quality Standards
- All consolidated files must include:
  - Complete metadata
  - Quality ratings
  - Source counts
  - Extraction timestamps
  - Proper relationships
  - Practical wisdom sections
