...
try:
    from .kabbalah_knowledge_database import (
        create_kabbalah_tradition, get_entry_by_id as get_kabbalah_entry_by_id,
        get_entries_by_tag as search_kabbalah_by_tag, get_all_entries as get_all_kabbalah_entries, 
        get_sefirot_data as get_ten_sefirot
    )
except ImportError:
    # Fallback functions if kabbalah database not available
    def create_kabbalah_tradition(): 
        return ProcessedTradition(
            name="kabbalah", 
            description="Fallback tradition - Kabbalah database not available", 
            total_entries=0,
            principles=[], 
            practices=[], 
            systems=[], 
    ...
    def get_kabbalah_entry_by_id(entry_id: str) -> Optional[KnowledgeEntry]: 
        return None
    def search_kabbalah_by_tag(tag: str) -> List[KnowledgeEntry]: 
        return []
    def get_all_kabbalah_entries() -> List[KnowledgeEntry]: 
        return []
    def get_ten_sefirot() -> List[Dict]: 
        return []

class UnifiedKnowledgeRetriever:
    """
    Unified knowledge retriever that provides access to all tradition databases
    """
    
    def __init__(self):
        """Initialize the unified knowledge retriever"""
